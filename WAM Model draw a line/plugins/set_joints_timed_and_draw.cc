...
namespace gazebo
{   
  class SetJointsTimedAndDraw : public ModelPlugin
  {
    public: void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)                
    {
      ...
      this->updateConnection = event::Events::ConnectWorldUpdateBegin(boost::bind(&SetJointsTimedAndDraw::OnUpdate, this, _1));
      std::ifstream joint_coordinate_commands;
      std::string path_to_joint_coord_file =  common::SystemPaths::Instance()->FindFileURI("model://WAM Model draw a line/forGazebo");
	...
	std::cout << "\n Gazebo found the joint coordinate list file at " << path_to_joint_coord_file << std::endl;
      ...
      {
	std::cout << "\n Gazebo failed to find the joint coordinate list file. There should be a file named forGazebo consisting of 7 CSVs per line in the \"WAM Model draw a line\" folder."
    ...
    {    
      const math::Vector3 st(1.0, 1.0, 1.0);
            const math::Vector3 en(0.0, 0.0, 0.0);
      
    rendering::Scene::DrawLine(st&, en&, "nnaname");
...
// )		
  ...
  GZ_REGISTER_MODEL_PLUGIN(SetJointsTimedAndDraw);
}
